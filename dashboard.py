...
<style>
    ...
    .card-lethal .symptom-tag {
        background: rgba(239,68,68,0.1);
        color: #fca5a5;
        border-color: rgba(239,68,68,0.2);
    }
    .card-notlethal .symptom-tag {
...
diseases = [
    {
        ...
        "vaccine": "Vacina Tríplice Viral (SCR)"
    },
    {
        "name": "Poliomielite",
        "emoji": "🦵",
    ...
    },
    {
        ...
        "description": "Infecção bacteriana grave causada por toxinas. A toxina afeta o sistema nervoso, podendo causar morte por parada respiratória.",
        "vaccine": "Vacina Tríplice Bacteriana (DTPa)"
    },
    {
        "name": "Hepatite B",
        "emoji": "🫀",
        ...
        "vaccine": "Vacina Hepatite B (3 doses)"
    },
    {
        "name": "Febre Amarela",
        "emoji": "🟡",
        "lethal": True,
        "symptoms": ["Febre", "Icterícia", "Sangramento", "Vômito intenso", "Insuficiência hepática"],
        ...
        "vaccine": "Vacina Febre Amarela (dose única)"
    },
    {
        ...
        "vaccine": "Vacina Influenza (anual)"
    },
    {
        "name": "Catapora",
        "emoji": "💢",
        ...
        "vaccine": "Vacina Varicela (Catapora)"
    },
    {
        "name": "Caxumba",
        "emoji": "😷",
        "lethal": False,
        "symptoms": ["Inchaço nas parótidas", "Dificuldade de mastigar", "Febre", "Dor de cabeça", "Fadiga"],
...
with st.sidebar:
    ...
    <div style='text-align:center; padding: 20px 0 10px;'>
        ...
        <div style='color:#a5b4fc; font-weight:700; font-size:1.1em;'>Saúde & Vacinação</div>
        <div style='color:#64748b; font-size:0.8em;'>Dashboard Brasil</div>
    </div>
    """, unsafe_allow_html=True)

    st.markdown("---")
    st.markdown("<div style='color:#94a3b8; font-size:0.85em; margin-bottom:8px;'>🔗 Links Rápidos</div>", unsafe_allow_html=True)
    st.markdown("""
    <a class='sidebar-link' href='https://meususdigital.saude.gov.br' target='_blank'>
        📍 Posto de Saúde Mais Próximo
    </a>
    <a class='sidebar-link' href='https://www.gov.br/saude/pt-br/assuntos/saude-de-a-a-z/v/vacinacao' target='_blank'>
        💉 Calendário Nacional de Vacinação
    </a>
    <a class='sidebar-link' href='https://www.gov.br/saude/pt-br' target='_blank'>
        🏥 Ministério da Saúde
    </a>
    """, unsafe_allow_html=True)

    st.markdown("---")
    st.markdown("<div style='color:#94a3b8; font-size:0.85em; margin-bottom:8px;'>🔍 Filtrar Doenças</div>", unsafe_allow_html=True)
    ...
    st.markdown("---")
    st.markdown("""
    <div style='color:#475569; font-size:0.75em; text-align:center; padding-top:10px;'>
        Dados: Ministério da Saúde BR<br>
        Atualizado: 2024
    </div>
    """, unsafe_allow_html=True)
...
# ─── KPI Stats ────────────────────────────────────────────────────────────────
df = load_data()
total = df["Doses_aplicadas"].sum()

col1, col2, col3, col4 = st.columns(4)
with col1:
    st.markdown(f"""
    <div class='stat-card'>
        <div class='stat-number'>89,2M</div>
        <div class='stat-label'>💉 Doses Aplicadas (Total)</div>
    </div>""", unsafe_allow_html=True)
with col2:
    st.markdown("""
    <div class='stat-card'>
        <div class='stat-number'>9</div>
        <div class='stat-label'>🦠 Doenças Cobertas</div>
    </div>""", unsafe_allow_html=True)
with col3:
    st.markdown("""
    <div class='stat-card'>
...
with col4:
    ...
    <div class='stat-card'>
        ...
        <div class='stat-label'>🎯 Meta de Cobertura SUS</div>
    </div>""", unsafe_allow_html=True)
...
if not filtered:
    ...
    for i, disease in enumerate(filtered):
        col = cols[i % 3]
        card_class = "card-lethal" if disease["lethal"] else "card-notlethal"
        badge_class = "badge-lethal" if disease["lethal"] else "badge-notlethal"
        badge_text = "⚠️ Potencialmente Letal" if disease["lethal"] else "✅ Geralmente Não Letal"

        symptoms_html = "".join([f"<span class='symptom-tag'>{s}</span>" for s in disease["symptoms"]])

        with col:
            st.markdown(f"""
            <div class='disease-card {card_class}'>
                <span class='card-emoji'>{disease['emoji']}</span>
                <div class='card-disease-name'>{disease['name']}</div>
                <span class='{badge_class}'>{badge_text}</span>
                <div class='symptoms-title'>Principais Sintomas</div>
                <div>{symptoms_html}</div>
                <div class='card-description'>{disease['description']}</div>
                <div style='margin-top:12px; padding-top:10px; border-top:1px solid rgba(255,255,255,0.06);'>
                    <span style='color:#64748b; font-size:0.75em;'>💉 Vacina: </span>
                    <span style='color:#a5b4fc; font-size:0.8em; font-weight:500;'>{disease['vaccine']}</span>
                </div>
            </div>
            """, unsafe_allow_html=True)
...
<div class='vaccine-banner'>
    <div class='vaccine-title'>🛡️ A Vacinação Salva Vidas</div>
    ...
    <div class='vaccine-points'>
        ...
        <div class='vaccine-point'>👶 <strong>Proteção dos Vulneráveis</strong><br>Crianças, idosos e gestantes dependem da vacinação de toda a população para se protegerem.</div>
...
<div class='cta-container'>
    ...
    <div style='color:#475569; font-size:0.82em; margin-top:12px;'>
        Acesse o <strong style='color:#6ee7b7;'>Meus Dados de Saúde Digital (Meu SUS Digital)</strong> 
...
coverage = {
    "Sudeste": 88, "Sul": 84, "Centro-Oeste": 76, "Nordeste": 72, "Norte": 65
}
...
bar_colors = [colors_cov[i] for i in range(len(regions))]
...
# Add goal line at 95%
fig_cov.add_vline(x=95, line_dash="dash", line_color="rgba(239,68,68,0.6)", annotation_text="Meta: 95%",
